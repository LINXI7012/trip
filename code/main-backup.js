...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (currentPage === 'home') {
        ...
        initExplorePage();
    } else if (currentPage === 'plans' || body.classList.contains('plans')) {
...
function initPlansPage() {
    ...
    function populatePlans(page) {
        ...
        if (currentPagePlans.length > 0) {
            ...
            const planCards = plansContainer.querySelectorAll('.plan-card');
    ...
    function updatePlanProgress(planId) {
        console.log('Updating plan progress for plan ID:', planId);
        ...
        console.log(`Progress calculation: ${completedTasks}/${totalTasks} tasks completed (${completionPercentage}%)`);
...
function showEditGuideModal(plan) {
    ...
    if (!titleNotesSection) {
        ...
        titleNotesSection.innerHTML = `
            ...
            <div class="form-group">
                ...
                <input type="date" id="startDateInput" class="form-control" value="">
            </div>
            <div class="form-group">
                <label for="endDateInput">结束日期</label>
                <input type="date" id="endDateInput" class="form-control" value="">
            </div>
    ...
    // 填充标题和备注字段
    document.getElementById('guideTitleInput').value = guideData.title || plan.destination || '';
    document.getElementById('guideNotesInput').value = guideData.notes || '';
    
    // 填充日期字段
    document.getElementById('startDateInput').value = plan.startDate || '';
    document.getElementById('endDateInput').value = plan.endDate || '';
    
    // 清除所有先前的事件监听器
    const closeBtn = document.querySelector('.edit-guide-close');
    const deleteBtn = document.getElementById('deleteGuideBtn');
    const modifyBtn = document.getElementById('modifyItineraryBtn');
    const backToEditBtn = document.getElementById('backToEditBtn');
    const backToDaysBtn = document.getElementById('backToDaysBtn');
    const saveDayBtn = document.getElementById('saveDayBtn');
    const cancelDeleteBtn = document.getElementById('cancelDeleteBtn');
    const confirmDeleteBtn = document.getElementById('confirmDeleteBtn');
    const saveTitleNotesBtn = document.getElementById('saveTitleNotesBtn');
    
    // 克隆元素以移除所有事件监听器
    if (closeBtn) closeBtn.replaceWith(closeBtn.cloneNode(true));
    if (deleteBtn) deleteBtn.replaceWith(deleteBtn.cloneNode(true));
    if (modifyBtn) modifyBtn.replaceWith(modifyBtn.cloneNode(true));
    if (backToEditBtn) backToEditBtn.replaceWith(backToEditBtn.cloneNode(true));
    if (backToDaysBtn) backToDaysBtn.replaceWith(backToDaysBtn.cloneNode(true));
    if (saveDayBtn) saveDayBtn.replaceWith(saveDayBtn.cloneNode(true));
    if (cancelDeleteBtn) cancelDeleteBtn.replaceWith(cancelDeleteBtn.cloneNode(true));
    if (confirmDeleteBtn) confirmDeleteBtn.replaceWith(confirmDeleteBtn.cloneNode(true));
    if (saveTitleNotesBtn) saveTitleNotesBtn.replaceWith(saveTitleNotesBtn.cloneNode(true));
    
    // 重新获取DOM元素引用
    const closeBtn2 = document.querySelector('.edit-guide-close');
    const deleteBtn2 = document.getElementById('deleteGuideBtn');
    const directDeleteBtn = document.getElementById('directDeleteBtn');
    const modifyBtn2 = document.getElementById('modifyItineraryBtn');
    const backToEditBtn2 = document.getElementById('backToEditBtn');
    const backToDaysBtn2 = document.getElementById('backToDaysBtn');
    const saveDayBtn2 = document.getElementById('saveDayBtn');
    const cancelDeleteBtn2 = document.getElementById('cancelDeleteBtn');
    const confirmDeleteBtn2 = document.getElementById('confirmDeleteBtn');
    const saveTitleNotesBtn2 = document.getElementById('saveTitleNotesBtn');
    ...
    if (directDeleteBtn) {
        ...
        directDeleteBtn.addEventListener('click', function() {
            ...
            console.log('转换后的ID:', numericPlanId, '当前存储的攻略数量:', savedGuides.length);
            
            // 找到并移除攻略
            const guideIndex = savedGuides.findIndex(guide => Number(guide.id) === numericPlanId);
            console.log('找到的攻略索引:', guideIndex);
            ...
            if (guideIndex !== -1) {
                ...
                // 保存更新后的攻略
                localStorage.setItem('savedGuides', JSON.stringify(savedGuides));
                console.log('攻略已从localStorage中直接删除，剩余攻略数量:', savedGuides.length);
    ...
    if (modifyBtn2) {
        modifyBtn2.addEventListener('click', function() {
            console.log('修改按钮被点击');
            showDaySelection(plan);
        });
    }
    
    if (backToEditBtn2) {
        backToEditBtn2.addEventListener('click', function() {
            document.querySelector('.edit-section').style.display = 'block';
            document.querySelector('.day-edit-section').style.display = 'none';
        });
    }
    ...
    if (backToDaysBtn2) {
        backToDaysBtn2.addEventListener('click', function() {
    ...
    if (saveDayBtn2) {
        saveDayBtn2.addEventListener('click', function() {
            const planId = modal.getAttribute('data-plan-id');
    ...
    if (confirmDeleteBtn2) {
        confirmDeleteBtn2.addEventListener('click', function() {
            console.log('确认删除按钮被点击');
            const planId = modal.getAttribute('data-plan-id');
            console.log('要删除的计划ID:', planId);
            if (typeof deleteGuide === 'function') {
                deleteGuide(planId);
            } else {
                console.error('deleteGuide函数未定义');
            }
        });
    }
    
    if (saveTitleNotesBtn2) {
        saveTitleNotesBtn2.addEventListener('click', function() {
            const planId = modal.getAttribute('data-plan-id');
            ...
            saveTitleNotes(planId, title, notes, startDate, endDate);
        });
    }
 ...
 */
function saveTitleNotes(planId, title, notes, startDate, endDate) {
    // 获取保存的攻略
    let savedGuides = JSON.parse(localStorage.getItem('savedGuides') || '[]');
    ...
    if (guideIndex !== -1) {
        ...
        // 更新日期
        savedGuides[guideIndex].startDate = startDate;
        savedGuides[guideIndex].endDate = endDate;
 ...
 */
function showPlanDetails(plan) {
    console.log('Showing plan details for:', plan.destination);
    ...
    const completedTasks = plan.tasks.filter(task => task.completed).length;
    const completionPercentage = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
    console.log(`Initial progress calculation: ${completedTasks}/${totalTasks} tasks completed (${completionPercentage}%)`);
            ...
            <div class="plan-details-body">
                <div class="plan-details-section">
                    ...
                    <div class="plan-progress">
                        ...
                        <div class="progress-text">${completionPercentage}% 完成</div>
                    </div>
                </div>
                <div class="plan-details-section">
                    <h3>准备任务</h3>
    ...
    detailsHTML += `
            ...
            <div class="plan-details-footer">
                <button class="btn btn-small btn-outline edit-plan" data-plan-id="${plan.id}">编辑计划</button>
                <button class="btn btn-small btn-primary add-task-btn">添加任务</button>
            </div>
        </div>
    `;
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            console.log('Task checkbox changed, taskId:', taskId, 'checked:', this.checked);
            ...
            // If this is a saved guide, update localStorage
            if (plan.guideData) {
                const savedGuides = JSON.parse(localStorage.getItem('savedGuides') || '[]');
                const guideIndex = savedGuides.findIndex(g => g.id === plan.id);
                if (guideIndex !== -1) {
                    // If the plan has tasks in the saved guide, update it
                    if (!savedGuides[guideIndex].tasks) {
                        savedGuides[guideIndex].tasks = plan.tasks;
                    } else {
                        const savedTaskIndex = savedGuides[guideIndex].tasks.findIndex(t => t.id === taskId);
                        if (savedTaskIndex !== -1) {
                            savedGuides[guideIndex].tasks[savedTaskIndex].completed = this.checked;
                        }
                    }
                    localStorage.setItem('savedGuides', JSON.stringify(savedGuides));
                }
            ...
            // Update this item's class to reflect completion status
            if (this.checked) {
                this.closest('.task-item').classList.add('completed');
            } else {
                this.closest('.task-item').classList.remove('completed');
    ...
    // Add event listeners for task deletion
    const deleteButtons = detailsContainer.querySelectorAll('.task-delete-btn');
                ...
                if (plan.guideData) {
                    ...
                    if (guideIndex !== -1) {
                        if (!savedGuides[guideIndex].tasks) {
                            ...
                            const savedTaskIndex = savedGuides[guideIndex].tasks.findIndex(t => t.id === taskId);
                            if (savedTaskIndex !== -1) {
                                savedGuides[guideIndex].tasks.splice(savedTaskIndex, 1);
                            }
                        }
                        localStorage.setItem('savedGuides', JSON.stringify(savedGuides));
                    }
    ...
    const newTaskInput = detailsContainer.querySelector('#newTaskInput');
    const addTaskBtn = detailsContainer.querySelector('#addTaskBtn');
    
    if (newTaskInput && addTaskBtn) {
        // Add task on button click
        addTaskBtn.addEventListener('click', function() {
            addNewTask(plan, newTaskInput.value);
        });
        
        // Add task on Enter key
        newTaskInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                addNewTask(plan, this.value);
            }
        });
    }
    
    // Add event listener for edit button
    const editBtn = detailsContainer.querySelector('.edit-plan');
    if (editBtn) {
    ...
    // Add event listener for view full guide button
    const viewGuideBtn = detailsContainer.querySelector('.view-full-guide');
    if (viewGuideBtn && plan.guideData) {
        viewGuideBtn.addEventListener('click', function() {
            showTravelGuideDetails(plan.guideData);
        });
    }
    ...
    function addNewTask(plan, taskText) {
        ...
        if (taskText) {
            ...
            if (plan.guideData) {
                ...
                if (guideIndex !== -1) {
                    if (!savedGuides[guideIndex].tasks) {
                        ...
                        savedGuides[guideIndex].tasks.push(newTask);
                    }
                    localStorage.setItem('savedGuides', JSON.stringify(savedGuides));
                }
            }
            ...
            const tasksList = detailsContainer.querySelector('.tasks-list');
            const taskItem = document.createElement('li');
            taskItem.className = 'task-item';
            taskItem.innerHTML = `
                <div class="task-checkbox">
                    <input type="checkbox" id="task-${newTaskId}" data-task-id="${newTaskId}">
                    <span class="checkmark"></span>
                    <span class="task-text">${taskText}</span>
                </div>
                <button class="task-delete-btn" data-task-id="${newTaskId}">
                    <i class="fas fa-trash"></i>
                </button>
            `;
